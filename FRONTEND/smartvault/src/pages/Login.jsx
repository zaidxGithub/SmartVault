    ...
    <div className={`${themeBg} ${themeText} min-h-screen flex items-center justify-center px-4 py-8 transition-colors duration-300`}>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-medium">{isLogin ? 'Enterprise Secure Storage Platform' : 'Create Your Account Securely'}</p>
        </div>
        ...
        <div className={cardClass}>
          ...
          <div className={`${theme === 'dark' ? 'bg-slate-700 border-slate-600' : 'bg-gray-50 border-gray-200'} border-b`}>
            <div className="flex">
              <button onClick={() => setIsLogin(true)} className={tabActiveClass(isLogin)}>Sign In</button>
              <button onClick={() => setIsLogin(false)} className={tabActiveClass(!isLogin)}>Create Account</button>
            </div>
          </div>
